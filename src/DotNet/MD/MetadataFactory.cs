...
namespace dnlib.DotNet.MD {
	...
	public static class MetadataFactory {
		...
		internal static MetadataBase Load(IPEImage peImage, CLRRuntimeReaderKind runtime) { return Create(peImage, runtime, true); }
		...
		public static Metadata CreateMetadata(IPEImage peImage) { return CreateMetadata(peImage, CLRRuntimeReaderKind.CLR); }
		...
		/// <returns>A new <see cref="Metadata"/> instance</returns>
		public static Metadata CreateMetadata(IPEImage peImage, CLRRuntimeReaderKind runtime) { return Create(peImage, runtime, true); }
		...
		/// <returns>A new <see cref="Metadata"/> instance</returns>
		public static Metadata CreateMetadata(IPEImage peImage, bool verify) { return CreateMetadata(peImage, CLRRuntimeReaderKind.CLR, verify); }
		...
		static MetadataBase Create(IPEImage peImage, CLRRuntimeReaderKind runtime, bool verify) {
			...
			try {
				...
				switch (GetMetadataType(mdHeader.StreamHeaders, runtime)) {
					case MetadataType.Compressed: md = new CompressedMetadata(peImage, cor20Header, mdHeader, runtime); break;
					case MetadataType.ENC: md = new ENCMetadata(peImage, cor20Header, mdHeader, runtime); break;
                    default: throw new BadImageFormatException("No #~ or #- stream found");
				}
				md.Initialize(null);
		...
		internal static MetadataBase CreateStandalonePortablePDB(DataReaderFactory mdReaderFactory, bool verify) {
			...
			try {
				...
				switch (GetMetadataType(mdHeader.StreamHeaders, runtime)) {
					case MetadataType.Compressed: md = new CompressedMetadata(mdHeader, true, runtime); break;
					case MetadataType.ENC: md = new ENCMetadata(mdHeader, true, runtime); break;
                    default: throw new BadImageFormatException("No #~ or #- stream found");
				}
