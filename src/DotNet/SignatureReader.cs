...
namespace dnlib.DotNet {
	...
	public struct SignatureReader {
		...
		public static CallingConventionSig ReadSig(ModuleDefMD module, byte[] signature) {
			return ReadSig(module, module.CorLibTypes, ByteArrayDataReaderFactory.CreateReader(signature), new GenericParamContext());
        }
		...
		public static CallingConventionSig ReadSig(ISignatureReaderHelper helper, ICorLibTypes corLibTypes, byte[] signature) {
			return ReadSig(helper, corLibTypes, ByteArrayDataReaderFactory.CreateReader(signature), new GenericParamContext());
        }
		...
		public static TypeSig ReadTypeSig(ModuleDefMD module, byte[] signature) {
			return ReadTypeSig(module, module.CorLibTypes, ByteArrayDataReaderFactory.CreateReader(signature), new GenericParamContext());
        }
		...
		public static TypeSig ReadTypeSig(ISignatureReaderHelper helper, ICorLibTypes corLibTypes, byte[] signature) {
			return ReadTypeSig(helper, corLibTypes, ByteArrayDataReaderFactory.CreateReader(signature), new GenericParamContext());
        }
		...
		FieldSig ReadField(CallingConvention callingConvention) { return new FieldSig(callingConvention, ReadType()); }
		...
		MethodSig ReadMethod(CallingConvention callingConvention) { return ReadSig(new MethodSig(callingConvention)); }
		...
		PropertySig ReadProperty(CallingConvention callingConvention) { return ReadSig(new PropertySig(callingConvention)); }
		...
		T ReadSig<T>(T methodSig) where T : MethodBaseSig {
            ...
            uint numParams;
            if (!reader.TryReadCompressedUInt32(out numParams))
				return null;
